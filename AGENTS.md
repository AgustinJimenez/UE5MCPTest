...
- **Struct Type Mismatches (2026-01-30)**: When a blueprint struct (S_*) is replaced with a C++ struct (FS_*), and the blueprint struct is used as a Map/Array/Set value type in another struct, you must manually fix the reference in the editor. The `modify_struct_field` MCP command **cannot** handle this automatically due to fundamental UE5 reflection limitations (C++ structs don't appear in object system until instantiated).
