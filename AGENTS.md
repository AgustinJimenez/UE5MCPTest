...
- **Known Issue - GameMode Reset After Clean/Restart (2026-02-05)**: `WorldSettings.GameMode Override` can reset to `None` after `npm run clean:win` + restart, causing no pawn to spawn. **Fix**: set `GlobalDefaultGameMode=/Script/UETest1.GM_Sandbox` (and `GlobalDefaultServerGameMode`) in `Config/DefaultEngine.ini` under `[/Script/EngineSettings.GameMapsSettings]`.
...
- **Input vs Locomotion Debug Pattern (2026-02-04)**: If a feature key/input (e.g., Sprint) is detected and input flags toggle correctly, but movement state doesn't change (e.g., Gait stays Run, MaxWalkSpeed stays run value), treat it as a state-application/locomotion-logic issue rather than an input mapping issue. Verify both layers separately:
  - Input layer: key state, action trigger, input-state booleans (`WantsToSprint`, etc.)
  - Locomotion layer: gait enum, max speed, acceleration, and the function graph path that applies those values (`GetDesiredGait`, `CanSprint`, speed calculators)
  - During BP->C++ migration, temporary runtime fallback overrides are acceptable to keep gameplay functional while blueprint graph logic is repaired.
...
- **Build system cache note (2026-02-04)**: After adding/removing/splitting plugin `.cpp` files, UnrealBuildTool can keep a stale module makefile and miss new translation units. If linker errors appear for methods that clearly exist, run `npm run clean:win` (or delete `Intermediate/` + `Binaries/`) and rebuild to force makefile regeneration.
- **WorldSettings GameMode reset after clean/restart (2026-02-04)**: After `npm run clean:win` + restart, some levels can lose their GameMode override and spawn only the editor camera. Fix by setting `WorldSettings.DefaultGameMode` (or level override) back to `GM_Sandbox` and saving the level. MCP can read/set this via `read_actor_properties`/`set_actor_properties` on `WorldSettings`.
