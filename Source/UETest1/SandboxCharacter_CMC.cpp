...
ASandboxCharacter_CMC::ASandboxCharacter_CMC()
{
	...
	// Speed defaults (matched from BP CDO)
...
void ASandboxCharacter_CMC::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		if (IA_Sprint)
		{
			...
			EnhancedInputComponent->BindAction(IA_Sprint, ETriggerEvent::Completed, this, &ASandboxCharacter_CMC::OnSprintReleased);
		}
		...
		if (IA_Aim)
		{
			...
			EnhancedInputComponent->BindAction(IA_Aim, ETriggerEvent::Completed, this, &ASandboxCharacter_CMC::OnAimReleased);
		}
	}
}
...
void ASandboxCharacter_CMC::Tick(float DeltaTime)
{
	...
	// Update SteeringTime: accumulates while there is movement input, resets when idle
	const bool bHasInput = HasMovementInputVector();
	if (bHasInput)
	{
	...
	// Push MovementMode/Gait to AnimInstance (bypasses BP enum type mismatch in ABP PropertyAccess)
	UpdateAnimInstanceEnums();
}

void ASandboxCharacter_CMC::UpdateAnimInstanceEnums()
{
	...
	// Compute current MovementMode: 0=OnGround, 1=InAir (matches E_MovementMode enum order)
	uint8 CurrentMovementMode = 0;
	if (UCharacterMovementComponent* CMC = GetCharacterMovement())
	{
		CurrentMovementMode = CMC->IsFalling() ? 1 : 0;
	}

	// Current Gait: 0=Walk, 1=Run, 2=Sprint (matches E_Gait enum order)
	uint8 CurrentGait = static_cast<uint8>(Gait);

	// Write LastFrame values (from previous tick), then current values
...
FS_CharacterPropertiesForCamera ASandboxCharacter_CMC::Get_PropertiesForCamera_Implementation()
{
	...
	// CameraMode: derived from input state (matches original BP logic)
	if (CharacterInputState.WantsToAim)
	{
...
void ASandboxCharacter_CMC::OnSprintReleased(const FInputActionValue& Value)
{
...
void ASandboxCharacter_CMC::OnJumpAction(const FInputActionValue& Value)
{
	...
	if (CachedTraversalLogic && !bIsFalling && !CachedTraversalLogic->DoingTraversalAction && !IsRagdolling)
	{
		...
		// TryTraversalAction returns true when NO traversal happened (should jump instead)
		// Returns false when traversal was started (don't jump)
	...
	// Default: regular jump (falling, ragdolling, or no traversal logic)
	Jump();
}
...
void ASandboxCharacter_CMC::OnAimReleased(const FInputActionValue& Value)
{
...
E_MovementDirection ASandboxCharacter_CMC::CalculateMovementDirection() const
{
	...
	// Calculate direction angle: 0=forward, ±180=backward, +90=right, -90=left
	const double Direction = UKismetAnimationLibrary::CalculateDirection(Vel, GetActorRotation());

	// 6-way direction bucketing (Game Animation Sample convention)
	// F:  -60 to  60
	// RL:  60 to  90   (forward-right diagonal)
	// RR:  90 to 135   (hard right)
	// B:  135 to 180 / -180 to -135
	// LL: -135 to -90  (hard left)
	// LR: -90  to -60  (forward-left diagonal)
